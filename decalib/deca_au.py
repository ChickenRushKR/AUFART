...
class DECA(nn.Module):
    ...
    def _create_model(self, model_cfg):
        ...
        self.GAT = GAT(nfeat=512, 
                ...
                nheads=8, 
                alpha=0.2,
                batchsize=cfg.dataset.batch_size)
        # self.FTD = FLAMETransformerDecoderHead()
    ...
    def encode(self, images_224,  use_detail=True):
        ...
        # else:
            # t1 = time.time()
        x, au, main_sc = self.AUNet(images_224)
        adj = torch.ones((main_sc.shape[0], main_sc.shape[1], main_sc.shape[1])).to(self.device)
        hprime = self.GAT(main_sc, adj)

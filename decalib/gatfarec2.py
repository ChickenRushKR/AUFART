...
from .models.encoders_au import AUDetector
...
class DECA(nn.Module):
    ...
    def _create_model(self, model_cfg):
        ...
        # self.AU_Detector = AUDetector().to(self.device)
    ...
    # @torch.no_grad()
    def encode(self, images_224,  use_detail=True, use_gnn=False):
        # if use_detail:
        ...
        if use_detail or use_gnn:
            ...
            x, afn, _ = self.AUNet(images_224, use_gnn=use_gnn)

        # full_rep = x.mean(dim=-2).view([self.batchsize, 1, -1])
        # feats = torch.concat([main_sc, full_rep], dim=1)
        x, parameters_224 = self.E_flame.forwardResNet(images_224)
        ...
        # codedict['pred_au'] = self.AU_Detector(parameters[:,:53])
    ...
    def model_dict(self):
        return {
            ...
            # 'AU_Detector': self.AU_Detector.state_dict(),

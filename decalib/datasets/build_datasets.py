...
class SelfDataset(Dataset):
    def __init__(self, K,  image_size, mediapipePath = 'mediapipe_landmark_embedding.npz'):
        ...
        self.source = ['/mnt/hdd/dataset/FFHQ/masks', '/mnt/hdd/dataset/CelebAHQ/masks', \
            '/mnt/hdd/dataset/VGGFace2_HQ/masks', '/mnt/hdd/dataset/Affwild/masks', '/mnt/hdd/dataset/BUPT/masks']
        # self.source = ['/mnt/hdd/dataset/Affwild/masks']

        self.allmasksFolder = (
                    glob(self.source[0] + '/*.npy') + glob(self.source[1] + '/*.npy') + glob(self.source[2] + '/*.npy') \
                    # + glob(self.source[2] + '/*.npy') + glob(self.source[3] + '/*.npy')
                    + glob(self.source[3] + '/*/*/*.npy') + glob(self.source[4] + '/*.npy')
        )
    ...
    def __getitem__(self, idx):
        ...
        # mask = cv2.resize(mask, (224, 224)).transpose(2, 0, 1)
        ...
        #     aus = np.load(au_path)
        #     au_list.append(aus)
        #     au_array = torch.from_numpy(np.array(au_list)).type(dtype=torch.float32)
        #     data_dict = {

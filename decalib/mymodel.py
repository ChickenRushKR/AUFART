...
class mymodel(nn.Module):
    def __init__(self, config=None, device='cuda'):
        ...
        self.face_detector = face_alignment.FaceAlignment(face_alignment.LandmarksType._2D, flip_input=False)
    ...
    def _setup_renderer(self, model_cfg):
        ...
        # self.render = SRenderY(self.image_size, flame_mask_file=model_cfg.flame_mask_path, obj_filename=model_cfg.topology_path, uv_size=model_cfg.uv_size,
        #                        rasterizer_type=self.cfg.rasterizer_type).to(self.device)
    ...
    def _create_model(self, model_cfg):
        ...
        self.n_movem = model_cfg.n_exp + 3  # exp + jaw pose
        self.param_dict = {i: model_cfg.get('n_' + i) for i in model_cfg.param_list}

        # encoders
        # self.E_flame = ResnetEncoder(outsize=self.n_param).to(self.device)
        self.VIT = ViT(image_size=224, patch_size=16, num_classes=53, dim=768, depth=6, heads=8,mlp_dim=1024,dropout=0.1,emb_dropout=0.1).to(self.device)
        self.E_flame = ResnetEncoder(outsize=self.n_param).to(self.device)
        ...
        if os.path.exists(model_path):
            ...
            util.copy_state_dict(self.VIT.state_dict(), checkpoint['VIT'])
    ...
    def landmark_detection(self, image):
        # print(image.shape)
        out = self.face_detector.get_landmarks(image[0].permute(2,1,0) * 255.)
        size = self.cfg.dataset.image_size
        out[0][:, :2] = out[0][:, :2] /size  * 2 - 1
        out = np.array(out)
        out = torch.tensor(out)
        return out

    def export_patches(self, images, landmarks):
        batch_size, num_landmarks, _ = landmarks.size()
        patch_size = 16
        halfpatch = int(patch_size / 2)

        # Initialize an empty tensor to store the patches
        patches = torch.zeros(batch_size, num_landmarks - 17, 3 * patch_size * patch_size)

        # Convert normalized landmarks to pixel coordinates
        landmarks = (landmarks + 1.0) * 112

        # Iterate through each batch and landmark
        for i in range(batch_size):
            for j in range(17, num_landmarks):
                landmark = landmarks[i, j]
                x, y = landmark[0].item(), landmark[1].item()

                # Convert to integers and ensure coordinates are within image bounds
                x, y = int(x), int(y)
                
                # Adjust to have the center of the patch
                x, y = max(patch_size // 2, min(x, 223 - patch_size // 2)), max(patch_size // 2, min(y, 223 - patch_size // 2))

                # Calculate zero-padding if the patch is not completely within the image bounds
                top_pad = max(0, patch_size // 2 - y)
                bottom_pad = max(0, y + patch_size // 2 - 223)
                left_pad = max(0, patch_size // 2 - x)
                right_pad = max(0, x + patch_size // 2 - 223)

                # Pad the image
                padded_image = F.pad(images[i:i+1], (left_pad, right_pad, top_pad, bottom_pad), value=0)

                # Extract a 32x32 patch around the landmark using indexing
                patch = padded_image[:, :, y-patch_size//2:y+patch_size//2, x-patch_size//2:x+patch_size//2]

                # patches[i, j] = patch
                patches[i, j - 17] = torch.flatten(patch)
        patches = patches.to(self.device)
        return patches


    # @torch.no_grad()
    def encode(self, images_224, lmk=None, use_detail=True):
        # if use_detail:
        ...
        # else:
            # t1 = time.time()
        if lmk == None:
            lmk = self.landmark_detection(images_224)
        patches = self.export_patches(images_224, lmk)
        parameters = self.VIT(patches)
        with torch.no_grad():
    ...
    def decode(self, codedict, rendering=True, iddict=None, vis_lmk=True, return_vis=True, use_detail=True,
        ...
        if return_vis:
            ...
            visdict = {
                ...
                # 'render_images': opdict['rendered_images'],

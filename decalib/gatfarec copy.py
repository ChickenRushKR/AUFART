...
from .models.GraphAttentionTransformer import GAT, GraphTransformerDecoder
...
class DECA(nn.Module):
    ...
    def _create_model(self, model_cfg):
        ...
        # self.AU_Encoder = AUEncoder(outsize=self.n_movem).to(self.device)
        self.GAT = GAT(nfeat=512, 
                nhid=512, 
                noutput=27, 
                dropout=0.0, 
                nheads=4, 
                alpha=0.2,
                batchsize=cfg.dataset.batch_size).to(self.device)
        self.FTD = FLAMETransformerDecoderHead().to(self.device)
        # self.FTD = GraphTransformerDecoder(512, 53)
        ...
        if os.path.exists(model_path):
            ...
            util.copy_state_dict(self.GAT.state_dict(), checkpoint['GAT']())
    ...
    #                                                         images=background, return_grid=False)
    ...
    #         if 'uv_texture_gt' in codedict.keys():
    #             opdict['uv_texture_gt'] = codedict['uv_texture_gt']

    #         ops_gt = self.render(verts, trans_verts, opdict['uv_texture_gt'], h=h, w=w, background=background)

    #         tempRenderImages = (opdict['rendered_images']*opdict['v_colors']).cpu().numpy()
    #         tempImages = images.cpu().numpy().copy()
    #         if use_detail:
    #             visdict = {
    #                 'inputs': images,
    #                 # 'rendered_image':images*(1-opdict['v_colors'])+ops_gt['images']*opdict['v_colors'],
    #                 # 'rendered_image':images*(1-opdict['v_colors'])+opdict['rendered_images']*opdict['v_colors'],
    #                 # 'rendered_image_detail':images*(1-opdict['v_colors'])+ops_detail['images']*opdict['v_colors'],
    ...
    #     else:
    #         return opdict
    # def encode(self, images,images_224,  use_detail=True):
    #     if use_detail:
    #         # use_detail is for training detail model, need to set coarse model as eval mode
    #         with torch.no_grad():
